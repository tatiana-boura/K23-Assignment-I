...
/*
call format:
./prog ../datasets/camera_specs/2013_camera_specs ../datasets/sigmod_medium_labelled_dataset.csv common-english-words.txt <bias> result.txt

Program's work flow:
1.Create .json lists of wordInfo & Vocabulary
2.Create tf-idf/BoW vectors  [we suggest to use the same vector type used in model training]
3.Set parameters (w,bias) from trained model (no training takes place during testing)
4.Predict class relations for the pairs in given W label set
*/
...
#include "./reading/parsing_functions.h" 
...
int main(int argc, char* argv[]){
	...
	printf("(we suggest to use the same vector type used in model training)\n");
	bool flag=true;
	char c;
	scanf("%c",&c);
	while(flag){
	...
	//-----------PREDICT-------------------------------------------------------------------------------------------------
	//create set arrays
	float** x_test_array = calloc(1,sizeof(float*));
	unsigned int* y_test_array = calloc(1,sizeof(unsigned int));
	unsigned int n;	 // number of differences (size of x and y array)
	create_x_y_array(&x_test_array,&y_test_array,ht,BUCKETSIZE, voc_size,&n);

	//set models parameters
	printf("\nSet model parameters\n");
	float bias = atoi(argv[4]);
	
	FILE* weights = fopen(argv[5], "r");
	if(weights == NULL){
		perror("Unable to open file :(");
		exit(-1);
	} 
	...
	*/
	...
	free(x_test_array); free(y_test_array);
