...
Για τις απαιτήσεις της τρίτης εργασίας υλοποιήθηκαν οι κατάλληλες συναρτήσεις έτσι ώστε η λίστα να μπορεί να λειτουργεί και ως ουρά (Queue) για την αποθήκευση των εργασιών που καλούνται να εκτελέσουν τα νήματα. Συγκεκριμένα, μπορούν να εφαρμοστεί η εισαγωγή στοιχείων με τη λογική append end καθώς και η αφαίρεση στοιχείου από την κεφαλή της λίστας (pop).
...
Αφότου διαβαστεί το αρχείο datasetW και δημιουργηθούν οι κλίκες και οι αντικλίκες, δημιουργούμε με τη συνάρτηση create_x_y_array έναν array X με τις απόλυτες διαφορές των tfidf ή BoW vectors των δύο εμπλεκόμενων json και έναν array Y που έχει τιμή 1 ή 0 εάν το ζεύγος ανήκει σε κλίκα ή αντικλίκα αντίστοιχα. Για το datasetW έχουμε 42535 ζευγή με αποτίμηση 1 και 299394 ζεύγη με αποτίμηση 0. Επειδή όμως παρατηρήσαμε πως τα μηδενικά είναι πολλά περισσότερα από τους άσσους και το training set είναι unbalanced, αποφασίσαμε να προσθέτουμε κάθε ζεύγος άσσων 7 φορές προκειμένου το πλήθος των μηδενικών και το πλήθος των άσσων να είναι περίπου ίσο.
...
--------------------------------------------------------------------------------------------------------------------------------
**Δεύτερο Εκτελέσιμο(Inference) - testing.c**
## Compile and run
> cd Project2
> make test_prog
> ./test_prog ../datasets/camera_specs/2013_camera_specs ../datasets/sigmod_medium_labelled_dataset.csv common-english-words.txt bias result.txt

όπου "bias" ο αριθμός κινητής υποδιαστολής που εκτυπώσει το prog.

Παρατήρηση: Εάν ο εξεταστής τρέξει το αρχικό και το inference πρόγραμμα με τον πρώτο τρόπο που δόθηκε στην αρχή, τροποποιείται η εντολή εκτέλεσης του inference σε 

> ./Project2/test_prog ./datasets/camera_specs/2013_camera_specs ./datasets/sigmod_medium_labelled_dataset.csv ./Project2/common-english-words.txt bias ./result.txt

## Λειτουργία 
Το πρόγραμμα 
-Διαβάζει το dataset_X που δίνεται ως είσοδος και οργανώνει κατάλληλα τις πληροφορίες των  αρχείων .json ενώ παράλληλα δημιουργεί το λεξιλόγιο (vocabulary).
-Διαθέτει από την εντολή εκτέλεσης την τιμή της παραμέτρου bias και τα βάρη των αντίστοιχων λέξεων του λεξιλογίου από το αρχείο result.txt . Οπότε Προσπερνά το κομμάτι της εκπαίδευσης του μοντέλου.
-Όμοια με το κομμάτι predict ου κυρίως προγράμματος, προβλέπει την σχέση '0' ή '1' για κάθε ένα από τα ζευγάρια που περιέχονται στο αρχείο των labels και συγκρίνει την πρόβλεψη με την πραγματική συσχέτιση των δύο προϊόντων.
 
## Παραδοχές
Για την ορθή λειτουργία του προγράμματος:
1) Όσον αφορά το dataset_X.
[κώδικας μεταξύ των σχολίων: '//--[ Α ]--' & '//--[ Β ]--']  
Βάση της αρχικής μας υλοποίησης, οφείλει να είναι το ίδιο με αυτό που δόθηκε στο κυρίως πρόγραμμα για την εκπαίδευση του μοντέλου. Σε διαφορετική περίπτωση ενδέχεται να διαφοροποιηθεί το λεξιλόγιο και να υπάρξει αναντιστoιχία των βαρών που έχουν υπολογιστεί κατά την εκπαίδευση του μοντέλου.
[κώδικας μεταξύ των σχολίων: '//--[ Β ]--' & '//--[ C ]--']  
Υπάρχει σχολιασμένη προσθήκη κώδικα για να λειτουργεί με είσοδο dataset X διαφορετικό από αυτό που δόθηκε στο κυρίως πρόγραμμα για την εκπαίδευση του μοντέλου. Το πρόγραμμα προσαρμόζει τις τιμές των βαρών που υπολογίστικαν για το λεξιλόγιο στις λέξεις του νέου λεξιλογίου που κατασκευάζεται βάση της νέας εισόδου.

Ανάλογα με την προβλεπόμενη χρήση αρκεί να αποσχολιαστεί το αντίστοιχο κομμάτι κώδικα.
Για την ορθή λειτουργία του προγράμματος το ένα από τα δύο κομμάτια θα πρέπει να βρίσκεται σε σχόλια.

2) στο dataset_W το κάθε ζεύγος οφείλει να διαθέτει ετικέτα (0,1) έτσι ώστε να προσδιοριστεί το ποσοστό επιτυχών προβλέψεων του μοντέλου.
